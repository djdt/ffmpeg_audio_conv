...
def parse_args(args):
    parser = argparse.ArgumentParser(
        description="Converts audio files using ffmpeg," " preserves dir structure.",
    ...
    parser.add_argument(
        ...
        help="Options to pass to ffmpeg.",
    )
    ...
    # Converter settings
    parser.add_argument(
        "-p", "--pretend", action="store_true", help="Perform no actions."
    )
    parser.add_argument(
        "-t", "--threads", type=int, default=4, help="Maximum number of threads used."
    )
...
def gather_files(args):
    ...
    for f in fileops.search_exts(args["indir"], args["inexts"]):
        ...
        if os.path.exists(t):
            ...
            if args["updatetags"] is not None and moddate >= args["updatetags"]:
...
if __name__ == "__main__":
    ...
    if not args["updateonly"]:
        failed, completed, convert_time = convert(convfiles, args, logger, killer)
    ...
    # Display end msg
    print(
        "Processed {} dirs, {} files in {:.2f} seconds.".format(
            fileops.count_dirs(args["indir"]), num_files, convert_time + update_time
        )
    )
    if not args["updateonly"]:
        print(
            "Convert: {} errors, {} skipped, {} converted.".format(
    ...
    if updated > 0:
        print(
            "Update: Updated tags in {} files, skipped {} files.".format(
